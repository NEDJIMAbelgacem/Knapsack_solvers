...
void Heuristics::loadDataSet(QString dataset_name) {
...
void Heuristics::init() {
    ...
    methodCB->addItem("value ordered");
    methodCB->addItem("density ordered");
    QObject::connect(methodCB, &QComboBox::currentTextChanged, [&](const QString& text) {
        if (text == "value ordered") method = 0;
        if (text == "density ordered") method = 1;
    });
}

void Heuristics::executeAlgorithm() {
    std::chrono::time_point<std::chrono::system_clock> start, end;

    QString dataset_name = dataSetCB->currentText();
    double sack_size = sackSizeSpinBox->value();
    QString method_str;
    if (method == 0) method_str = "value-ordered";
    if (method == 1) method_str = "density-ordered";
    QString command = MainWindow::methods_executable_paths[QString("heuristics")];
    command += QString(" 0 %1").arg((long long)sack_size);
    command += + " " + method_str + " " + MainWindow::datasets_file_path[dataset_name];
    qDebug() << command;
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (k != 0) solutionTableWidget->addItem(items_values[i], items_weight[i], k);
    }
