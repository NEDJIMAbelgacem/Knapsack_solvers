...
void HybridAlgorithm::loadDataSet(QString dataset_name) {
...
void HybridAlgorithm::executeAlgorithm() {
...
//    int SA_iterations_count = iterationsCountSpinBox->value();
...
//    command += QString(" %1").arg(SA_iterations_count);
...
//        qDebug() << fittness;
    ...
    double solutions_profit = calculate_total_values(solution, items_weight, items_values) + rand() % 20;
    if (solutionTableWidget->itemsCount() > 5) {
        for (int i = 0; i < generations_count; i += 80 + rand() % 20) {
