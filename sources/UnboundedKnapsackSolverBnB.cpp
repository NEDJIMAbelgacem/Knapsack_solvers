...
UnboundedKnapsackSolverBnB::UnboundedKnapsackSolverBnB(std::vector<double> _weights, std::vector<double> _values)
    : weights(_weights), values(_values) {
    items_sorted.resize(weights.size());
    items_values_sum = 0;
...
std::pair<double, std::vector<int>> UnboundedKnapsackSolverBnB::solve_impl(double sack_size, int curr_item, double cumulated, double best_possible_solution, double& best_solution_so_far) {
    ...
    if (sack_size >= weights[i]) {
        sub_problem2 = solve_impl(sack_size - weights[i], curr_item, cumulated + values[i], best_possible_solution, best_solution_so_far);
...
double UnboundedKnapsackSolverBnB::get_solution_total_weight(double sack_size) {
    double w = 0.0;
    for (int i : get_solution_items(sack_size)) w += weights[i];
    return w;
}

std::vector<int> UnboundedKnapsackSolverBnB::get_solution_items(double sack_size) {
    solve(sack_size);
    return sub_problem_solution_v[sack_size];
}

std::vector<double> UnboundedKnapsackSolverBnB::get_solution_values(double sack_size) {
    std::vector<double> res;
    for (int i : get_solution_items(sack_size)) res.push_back(values[i]);
    return res;
}

std::vector<double> UnboundedKnapsackSolverBnB::get_solution_weights(double sack_size) {
    std::vector<double> res;
